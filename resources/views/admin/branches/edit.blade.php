            ...
            <form action="{{ route('admin.branches.update', $branch) }}" method="POST">
                ...
                <div class="row mb-3">
                    <div class="col-md-6">
                        ...
                        <input type="time" class="form-control @error('opening_time') is-invalid @enderror"
                            ...
                            value="{{ old('opening_time', substr($branch->opening_time, 0, 5)) }}" required>
                        @error('opening_time')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="col-md-6">
                        ...
                        <input type="time" class="form-control @error('closing_time') is-invalid @enderror"
                            ...
                            value="{{ old('closing_time', substr($branch->closing_time, 0, 5)) }}" required>
                        @error('closing_time')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                ...
                <div class="mb-3">
                    ...
                    <textarea class="form-control @error('description') is-invalid @enderror" id="description" name="description"
                        rows="4">{{ old('description', $branch->description) }}</textarea>
                    @error('description')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
